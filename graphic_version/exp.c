...
double* RichardsonExtrapolation(int n, double segmentBegin, double segmentEnd, int k, double* y0) 
{
    ...
    for(int i = 0; i < EXTRAPOLATION_MATRIX_SIZE; i++)
    {
        ...
        matrix[i][0] = 1;
        for(int j = 1; j < EXTRAPOLATION_MATRIX_SIZE; j ++)
        {
...
void graphViewer(int n, double a, double b, int k, double* y0, double** result)
{
    ...
    FILE *graphic = fopen("plot_script.gnu", "w");
    double *outX = (double*)malloc(sizeof(double) * (k+1));

    for(int i = 0; i < k + 1; i++)
    {
        printf("%lf ",temp_a);
        fprintf(file, "%lf ",temp_a);
        outX[i] = temp_a;
        temp_a += h;

        for(int j = 0; j < n; j++)
        {
            fprintf(file, "%lf ", result[i][j]);
            printf("%lf ", result[i][j]);
